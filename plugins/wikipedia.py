...
class wikiPedia(Plugin):
    ...
    @register("en-US", ".*search (.*) search wikipedia|.*search (.*) search wiki|.*(wikipedia|wiki) (.*)")
    @register("fr-FR", ".*cherche (.*) sur wikipedia|.*cherche (.*) sur wiki|.*(wikipedia|wiki) (.*)")
    def askWiki(self, speech, language, regex):
        ...
        if jsonString != None:
            ...
            if response['query']['pages'].keys()[0] > 0:
                ...
                #elif language=="fr-FR":
                #    self.say(u'Je n\'ai rien trouv sur Wikipedia pour \"{0}\"'.format(searchString))
                else:
               ...
               # elif language=="fr-FR":
               #     self.say(u 'arrive pas econnecteripedia.')
                else:
